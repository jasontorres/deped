...
const TWEAKS_STYLE = `
  ...
  .twk-fab{position:fixed;right:20px;bottom:20px;z-index:2147483645;width:36px;height:36px;
    border-radius:50%;border:none;background:rgba(0,0,0,.78);color:#fff;font-size:16px;
...
export function useTweaks<T extends TweakValues>(defaults: T): [T, (keyOrEdits: string | Partial<T>, val?: string | number | boolean) => void] {
  const [values, setValues] = useState<T>(defaults);
  const setTweak = useCallback((keyOrEdits: string | Partial<T>, val?: string | number | boolean) => {
...
export function TweaksPanel({ title = 'Tweaks', children }: { title?: string; children: ReactNode }) {
  ...
  return (
    <>
      ...
      {!open && <button className="twk-fab" onClick={() => setOpen(true)} aria-label="Open tweaks">⚙</button>}
      {open && (
...
function TweakRow({ label, value, children, inline = false }: { label: string; value?: string | number | null; children?: ReactNode; inline?: boolean }) {
  return (
